...
def drawParser(parser,color,image):
	...
	for span in parser.spans: 
		...
		subimg = SubImage(image,(int(boxes[1]) -1,int(boxes[2]) -1),(int(boxes[3]) +1 , int(boxes[4]) +1 ))
		print 'printing subimg'
		subimg.save_PNG('/tmp/sub_img_out_' + str(file_number) + '.png')
